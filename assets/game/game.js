(function(){
  ...
  const LB_KEY = 'unifrog_leaderboard';
  function loadLeaderboard(){
    try{ const raw = JSON.parse(localStorage.getItem(LB_KEY) || '[]'); return Array.isArray(raw) ? raw : []; }
    catch(e){ return []; }
  }
  function saveLeaderboard(list){
    try{ localStorage.setItem(LB_KEY, JSON.stringify(list.slice(0,5))); }catch(e){}
  }
  ...
  function addToLeaderboard(name, s){
    const list = loadLeaderboard();
    list.push({name: (name||'FROG').toUpperCase().slice(0,12), score: s});
  ...
  updateMuteBtn();
  renderBoard(startBoardList, loadLeaderboard());
  ...
  lbSaveBtn.addEventListener('click', (e)=>{
    e.stopPropagation();
    const name = (lbNameInput.value || 'FROG').trim();
    const flooredScore = Math.floor(score);
    const list = addToLeaderboard(name, flooredScore);
  ...
  function endGame(){
    ...
    renderBoard(endBoardList, board);
  ...
  function updatePowerChip(){
    if(!power){ powerIndicatorEl.innerHTML = ''; return; }
    const labels = {
    ...
    powerIndicatorEl.innerHTML = '<div class="power-chip show">' + iconHtml + '<span>' + label + '</span><div class="bar"><i style="width:' + pct + '%"></i></div></div>';
  }

  function updateComboChip(){
    if(comboCount < 2 || state !== 'playing'){ comboIndicatorEl.innerHTML = ''; return; }
    const m = comboMultiplier();
    comboIndicatorEl.innerHTML = '<div class="combo-chip show">🔥 COMBO ×' + m + '</div>';
  }
