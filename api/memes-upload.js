...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rate = await checkRateLimit(ipHash, { maxPerDay: 15, cooldownSeconds: 20 });
    if (!rate.allowed) {
    ...
    if (!ALLOWED_MIME.has(file.mimetype)) {
      return res.status(400).json({ error: 'Unsupported file type. Use PNG, JPG, GIF, or WEBP.' });
    }
    ...
    return res.status(201).json({ meme: rows[0] });
  } catch (err) {
    console.error('Upload error:', err);
    const message = err && err.message && err.message.includes('maxFileSize')
      ? 'File too large. Max size is 5MB.'
      : 'Something went wrong uploading your meme. Please try again.';
